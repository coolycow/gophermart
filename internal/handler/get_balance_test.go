...
func createTestAccrual(t *testing.T, srv service.BalanceTransactionService, userID int, orderNumber string, amount float32) *model.BalanceTransaction {
	t.Helper()

	balanceTransaction, err := srv.CreateAccrual(context.Background(), userID, orderNumber, amount)
	require.NoError(t, err)

	return balanceTransaction
}

func createTestWithdraw(t *testing.T, srv service.BalanceTransactionService, userID int, orderNumber string, amount float32) *model.BalanceTransaction {
	t.Helper()

	balanceTransaction, err := srv.CreateWithdraw(context.Background(), userID, orderNumber, amount)
	require.NoError(t, err)
...
func TestGetBalanceHandlerIntegration(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			router.GET("/api/user/balance", GetBalanceHandler(balanceTransactionService))
			...
			if tt.wantCode == http.StatusOK {
				...
				err = json.Unmarshal(resultBody, &balance)
				require.NoError(t, err)
				assert.Equal(t, balance.Current, tt.wantCurrent)
				assert.Equal(t, balance.Withdrawn, tt.wantWithdrawn)
			}
		})
	}
}
